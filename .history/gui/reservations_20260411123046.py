    ...
    def open_form(self, reservation_id=None):
        ...
        if reservation_id:
            row = db.execute_query(
                "SELECT client_id, chambre_id, date_arrivee, date_depart, statut FROM reservations WHERE id = %s",
        ...
        def save():
            ...
            try:
                datetime.strptime(arrivee, '%Y-%m-%d')
            ...
            if not reservation_id:
                ...
                """
                conflict = db.execute_query(query_check, (chambre_id, depart, arrivee), fetch_one=True)
                if conflict:
                    messagebox.showerror("Erreur", "La chambre est déjà réservée sur cette période.")
                    return

            if reservation_id:
                query = """UPDATE reservations SET client_id=%s, chambre_id=%s, date_arrivee=%s, date_depart=%s, statut=%s WHERE id=%s"""
                params = (client_id, chambre_id, arrivee, depart, statut, reservation_id)
            else:
                query = """INSERT INTO reservations (client_id, chambre_id, date_arrivee, date_depart, statut) VALUES (%s, %s, %s, %s, %s)"""
