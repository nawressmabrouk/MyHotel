...
class FacturesFrame(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        buttons = [
            ("➕ Ajouter", "#f39c12", self.add),
            ("✏️ Modifier", "#f39c12", self.edit),
            ("🗑️ Supprimer", "#f39c12", self.delete),
            ("🔄 Rafraîchir", "#f39c12", self.load_data),
            ("📎 Exporter CSV", "#f39c12", self.export_csv)
        ]
    ...
    def open_form(self, facture_id=None):
        ...
        # Liste des réservations - Version corrigée (sans CONCAT pour éviter l'erreur de collation)
        if facture_id:
        ...
        # Créer le dictionnaire des réservations avec une chaîne formatée manuellement
        reservations = {}
        if res_rows:
            for row in res_rows:
