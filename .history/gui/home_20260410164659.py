...
class HomeFrame(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        # === OVERLAY PRINCIPAL (grille pour mieux contrôler) ===
        ...
        overlay_frame.place(relx=0.5, rely=0.5, anchor="center", relwidth=0.9, relheight=0.92)
        
        # Utilisation de grid pour structurer le contenu
        overlay_frame.grid_rowconfigure(0, weight=0)  # header
        overlay_frame.grid_rowconfigure(1, weight=1)  # cards (expand)
        overlay_frame.grid_rowconfigure(2, weight=0)  # footer
        ...
        separator.grid(row=1, column=0, sticky="ew", padx=200, pady=5)

        # === GRILLE DES CARTES ===
        cards_container = ctk.CTkFrame(overlay_frame, fg_color="transparent")
        cards_container.grid(row=2, column=0, sticky="nsew", padx=50, pady=20)
        ...
        features = [
            {"title": "🛏️ Chambres", "desc": "Gérer les chambres :\najout, modification,\nsuppression et suivi\ndes statuts", "color": "#2ecc71", "tab": "Chambres"},
            {"title": "👥 Clients", "desc": "Carnet d'adresses complet :\najout, recherche,\nmodification et\nsuppression", "color": "#3498db", "tab": "Clients"},
            {"title": "📅 Réservations", "desc": "Planifiez les séjours,\nvérifiez la disponibilité,\ngérez les arrivées\net départs", "color": "#e67e22", "tab": "Réservations"},
            {"title": "💰 Factures", "desc": "Générez des factures,\ncalculez automatiquement\nle montant, exportez\nles données", "color": "#e74c3c", "tab": "Factures"}
        ]
        ...
        for i, feature in enumerate(features):
            ...
            # Icône
            icon_frame = ctk.CTkFrame(card, width=70, height=70, corner_radius=35, fg_color=feature["color"], bg_color="transparent")
            icon_frame.pack(pady=(20, 10))
            icon_frame.pack_propagate(False)
            icon_frame.bind("<Button-1>", lambda e, t=feature["tab"]: self.go_to_tab(t))
            
            icon_label = ctk.CTkLabel(icon_frame, text=feature["title"][0:2], font=("Arial", 32), text_color="white", bg_color="transparent")
            icon_label.pack(expand=True)
            icon_label.bind("<Button-1>", lambda e, t=feature["tab"]: self.go_to_tab(t))

            # Titre
            title_label = ctk.CTkLabel(card, text=feature["title"], font=("Arial", 22, "bold"), text_color=("#1f538d", "#3a7eb6"))
            ...
            # Description
            desc_label = ctk.CTkLabel(card, text=feature["desc"], font=("Arial", 12), justify="center", text_color=("gray30", "gray70"))
            desc_label.pack(pady=15, padx=15, fill="both", expand=True)
            desc_label.bind("<Button-1>", lambda e, t=feature["tab"]: self.go_to_tab(t))

            # Bouton
            btn = ctk.CTkButton(card, text="Accéder →", width=140, height=38, corner_radius=19,
                                fg_color=feature["color"], hover_color="#2c3e50",
                                font=("Arial", 13, "bold"), command=lambda t=feature["tab"]: self.go_to_tab(t))
            btn.pack(pady=(5, 25))

        # === PIED DE PAGE (maintenant bien visible) ===
