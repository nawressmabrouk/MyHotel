...
class FacturesFrame(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        # Style pour l'en-tête (plus épais et coloré)
        ...
        # Couleur au survol et sélection
        style.map("Treeview",
        ...
        v_scroll.configure(style="Vertical.TScrollbar")
    ...
    def load_data(self):
        ...
        if rows:
            for i, r in enumerate(rows):
                ...
                # Appliquer une couleur différente pour les lignes paires
                if i % 2 == 0:
    ...
    def open_form(self, facture_id=None):
        ...
        # Créer le dictionnaire des réservations
        reservations = {}
        if res_rows:
            for row in res_rows:
                label = f"{row['client_nom']} - Ch {row['chambre_numero']} ({row['date_arrivee']}→{row['date_depart']})"
                reservations[label] = row['id']
        
        res_labels = list(reservations.keys())

        # Vérifier s'il y a des réservations disponibles
        if not res_labels:
        ...
        if facture_id and fact:
            for label, rid in reservations.items():
                if rid == fact['reservation_id']:
        ...
        # Widgets
        ctk.CTkLabel(win, text="📋 Réservation :", font=("Arial", 13)).grid(row=0, column=0, padx=10, pady=10, sticky="e")
        res_combo = ctk.CTkComboBox(win, values=res_labels, state="readonly", width=450)
        res_combo.grid(row=0, column=1, padx=10, pady=10)
        if res_labels:
            res_combo.set(res_labels[0])

        ctk.CTkLabel(win, text="💰 Montant total (€) :", font=("Arial", 13)).grid(row=1, column=0, padx=10, pady=10, sticky="e")
        ...
        ctk.CTkLabel(win, text="📅 Date paiement :", font=("Arial", 13)).grid(row=2, column=0, padx=10, pady=10, sticky="e")
        ctk.CTkEntry(win, textvariable=date_paiement_var, width=200).grid(row=2, column=1, padx=10, pady=10, sticky="w")

        ctk.CTkLabel(win, text="💳 Mode paiement :", font=("Arial", 13)).grid(row=3, column=0, padx=10, pady=10, sticky="e")
        ...
        def update_montant_auto(*args):
            ...
            if label and label in reservations:
                ...
                if row and row['date_arrivee'] and row['date_depart']:
                    ...
                    date1 = datetime.strptime(str(row['date_arrivee']), '%Y-%m-%d')
                    date2 = datetime.strptime(str(row['date_depart']), '%Y-%m-%d')
        ...
        ctk.CTkButton(win, text="💾 Enregistrer", font=("Arial", 13, "bold"), fg_color="#2ecc71", hover_color="#27ae60", command=save).grid(row=4, column=0, columnspan=2, pady=20)
