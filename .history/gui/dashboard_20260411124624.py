...
class DashboardFrame(ctk.CTkFrame):
    ...
    def create_widgets(self):
        title = ctk.CTkLabel(self, text="Tableau de bord - Statistiques", font=("Arial", 24, "bold"),  text_color="#1a1a2e")
        title.pack(pady=20)
        ...
        # === BOUTON ACTUALISER (style harmonisé) ===
    ...
    def create_card(self, row, col, title_text, initial_value):
        card = ctk.CTkFrame(self.cards_frame, corner_radius=10, border_width=2, border_color="#e67e22")
        card.grid(row=row, column=col, padx=15, pady=15, sticky="nsew")
        self.cards_frame.grid_rowconfigure(row, weight=1)
        self.cards_frame.grid_columnconfigure(col, weight=1)

        label_title = ctk.CTkLabel(card, text=title_text, font=("Arial", 16, "bold"), text_color="#e67e22")
