...
class ChambresFrame(ctk.CTkFrame):
    ...
    prix_var = ctk.StringVar()  # Utiliser StringVar pour éviter les erreurs de conversion
    statut_var = ctk.StringVar(value="libre")

    if chambre_id:
        row = db.execute_query(
            "SELECT numero, type, prix_nuit, statut FROM chambres WHERE id = %s",
            (chambre_id,), fetch_one=True
        )
        if row:
    ...
    # Champs
    ctk.CTkLabel(win, text="Numéro :").grid(row=0, column=0, padx=10, pady=15, sticky="e")
    entry_numero = ctk.CTkEntry(win, textvariable=numero_var, width=200)
    entry_numero.grid(row=0, column=1, padx=10, pady=15)

    ctk.CTkLabel(win, text="Type :").grid(row=1, column=0, padx=10, pady=15, sticky="e")
    ctk.CTkComboBox(win, values=["simple", "double", "suite"], variable=type_var, width=200).grid(row=1, column=1, padx=10, pady=15)

    ctk.CTkLabel(win, text="Prix par nuit (€) :").grid(row=2, column=0, padx=10, pady=15, sticky="e")
    entry_prix = ctk.CTkEntry(win, textvariable=prix_var, width=200)
    entry_prix.grid(row=2, column=1, padx=10, pady=15)

    ctk.CTkLabel(win, text="Statut :").grid(row=3, column=0, padx=10, pady=15, sticky="e")
    ctk.CTkComboBox(win, values=["libre", "occupee", "nettoyage"], variable=statut_var, width=200).grid(row=3, column=1, padx=10, pady=15)
