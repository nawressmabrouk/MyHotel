...
class FacturesFrame(ctk.CTkFrame):
    ...
    def open_form(self, facture_id=None):
        ...
        if facture_id:
            fact = db.execute_query(
                "SELECT reservation_id, montant_total, date_paiement, mode_paiement FROM factures WHERE id = %s",
        ...
        if not res_rows and not facture_id:
        ...
        if facture_id and fact:
            res_rows_fact = db.execute_query("""
                SELECT r.id, c.nom as client_nom, ch.numero as chambre_numero,
                       r.date_arrivee, r.date_depart
                FROM reservations r
                JOIN clients c ON r.client_id = c.id
        ...
        def save():
            ...
            if facture_id:
                ...
                params = (reservation_id, montant_var.get(), date_paiement or None, mode_combo.get() or None, facture_id)
            else:
                query = "INSERT INTO factures (reservation_id, montant_total, date_paiement, mode_paiement) VALUES (%s, %s, %s, %s)"
                params = (reservation_id, montant_var.get(), date_paiement or None, mode_combo.get() or None)
