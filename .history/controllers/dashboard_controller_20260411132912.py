...
class DashboardController:
    ...
    def get_chiffre_affaires_mois():
        ...
        result = db.execute_query("""
            ...
            FROM factures 
            WHERE strftime('%Y', date_paiement) = %s 
            AND strftime('%m', date_paiement) = %s
        """, (str(current_year), str(current_month).zfill(2)), fetch_one=True)
        return result['total'] if result and result['total'] else 0
    ...
    def get_ca_par_mois(months=6):
        """Récupère le chiffre d'affaires des derniers mois"""
        result = []
        ...
        for i in range(months - 1, -1, -1):
            ...
            ca = db.execute_query("""
                ...
                WHERE strftime('%Y', date_paiement) = %s 
                AND strftime('%m', date_paiement) = %s
            """, (str(year), str(month).zfill(2)), fetch_one=True)
